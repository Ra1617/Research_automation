...
======================
Queries 3 LLMs (Gemini, Groq, OpenRouter) for 163 company parameters,
consolidates via majority vote, and writes to Excel with confidence scoring.
...
def print_summary(company_name: str, result: dict, validation: dict):
    ...
    if validation["errors"]:
        ...
        if len(validation["errors"]) > 5:
            print(f"    ... and {len(validation['errors'])-5} more (see audit log)")
    ...
    # Show low-confidence required fields
    low_required = [
...
def main():
    ...
    parser.add_argument("--output", type=str, default=OUTPUT_FILE, help="Output Excel path")
    ...
    # Run the LangGraph 3-LLM pipeline
    print(f"\n Starting LangGraph pipeline for {company_name}...")
    graph = get_graph()
    state = graph.invoke({"company_name": company_name})
    ...
    # Save to Excel
    write_to_excel(company_name, result, all_params, args.output)
    
    # Save audit log
    save_audit_log(company_name, result, raw_outputs)
